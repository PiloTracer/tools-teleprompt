...
**Status:** Approved (v1 M1–M7 shipped; **v1.2 M8 approved** 2026-05-23)  
**Version:** 1.2  
**Created:** 2026-05-21  
**Revised:** 2026-05-23 — M8 adaptive teleprompter (mic VAD, read zone, meta lines) per approved SPEC + amendment 01
**Advanced mode:** balanced (startup-leaning — single VPS, minimal ops)  
**Foundation snapshot:** plan-master-ready 2026-05-20 · foundation-complete yes · implementation-ready yes (M8 additive)  
...
Build a dockerized, 100% web teleprompter with device-first script storage, optional cross-device handoff (QR fragment, multi-QR, LAN one-shot, or ephemeral relay + OTP), PWA offline support, and sanitized markdown rendering. No accounts, no database. **M1–M7 shipped**; **M8** adds optional **adaptive teleprompter** (browser mic + VAD, read-zone scroll, meta-line skim). Top risks: XSS via markdown (R6), relay token/OTP abuse (R4–R5), localStorage quota (R7), mic permission UX (R13). References foundation doc 04, ADRs 001–006, and feature SPECs under `.work/features/`.
...
| G2 — Optional cross-device handoff | J2 (relay), J2b (QR), **J2c (LAN)**, **J2d (multi-QR)** complete in E2E |
| G3 — Privacy-first | No script content in server logs; relay delete-on-read; QR/LAN/multi-QR paths avoid Redis body |
...
| G7 — Adaptive present (optional) | User enables mic sync; read line stays upper-middle; pause on silence; fixed speed when sync off |
...
| FR-11 | LAN one-shot handoff: in-memory API token, single GET, no Redis | revise 20260521, ADR 006 | M7 |
| FR-12 | Multi-QR: sequential QRs reassemble payload client-side; no API body | revise 20260521, ADR 006 | M7 |
| FR-13 | Adaptive teleprompter: optional mic + VAD sync; read zone; pause on silence; meta-line 2× skim; settings + player mic | adaptive-teleprompter SPEC 20260523 | M8 |
...
| NFR-11 | Serverless handoff privacy | LAN/multi-QR: no script in logs; LAN map not persisted to Redis | M7 |
| NFR-12 | Mic privacy | Adaptive: audio device-local only; no upload; explicit permission UX | M8 |
...
| R9 | QR exceeds capacity | M6-T1: auto-fallback; M7-T8: separate QR encode vs fragment limits |
| R10 | Stale service worker | M4-T5: vite-plugin-pwa update strategy |
| R11 | Multi-QR chunk order/loss | M7-T3: sequence numbers + checksum; UX “scan N of M” |
| R12 | LAN handoff token leak on shared network | M7-T2: short TTL, single GET, rate limit |
| R13 | Mic permission denied / false VAD pause | M8-T4–T7: graceful fallback to fixed speed; debounce tuning; clear UX |
...
| U8 | Default proposed | 8192 bytes compressed fragment; QR encode limit ~2500 B — M7-T8 |
| U9 | **Resolved** 2026-05-21 | M7 approved: LAN one-shot + multi-QR (ADR 006) |
...
1. Desktop: compress + encode script in URL **fragment** (≤ QR encode limit).
2. Mobile: scan QR or open link; fragment parsed client-side only.
3. Script saved to localStorage; no API call for body.

### Data flow — LAN one-shot handoff (J2c) — M7

1. Desktop: `POST /api/v1/handoff/lan` → in-memory token (not Redis), TTL 120s.
2. Desktop shows URL (uses `PUBLIC_HOST` / hotspot IP).
3. Mobile: single `GET` → script once → key deleted from API memory.
4. Mobile persists to localStorage; enters player.

### Data flow — multi-QR handoff (J2d) — M7

1. Desktop: split compressed payload into N QR-safe chunks; show “1 of N”.
2. Mobile: scan each QR in order; client reassembles + validates.
3. Script saved to localStorage; no API call for body.
...
- **Adaptive (M8, opt-in):** mic sync keeps read line upper-middle; defaults off; fixed speed when sync off.
...
| `/handoff/create` | Choose handoff mode (QR / multi-QR / LAN / relay) |
| `/handoff/receive` | QR / multi-QR fragment consume |
| `/handoff/claim/:token` | OTP entry (relay only) |
| `/handoff/lan/:token` | LAN one-shot fetch (M7) |
| `/settings` | Speed defaults, theme, adaptive + auto-sync toggles (M8), help |
...
| M6-T9 | `@code-verify milestone` prep; global acceptance checklist | `.work/plans/NEXT.md` | §20 | S | done 2026-05-21 |

---

### M7: Serverless large handoff (LAN + multi-QR)

**Objective:** Cross-device transfer for large scripts without Redis relay — LAN one-shot (in-memory API) and multi-QR sequence.

**Scope in:** ADR 006, LAN API + UI, multi-QR chunk encode/decode, handoff mode selection, E2E, SPEC amendments  
**Scope out:** WebRTC (deferred v2.1), Redis changes, filesystem uploads

**Dependencies:** M6 complete; owner approved M7 2026-05-21

**Deliverables:** J2c LAN journey; J2d multi-QR journey; updated handoff UX fallback chain

**Acceptance criteria:** LAN handoff on hotspot IP; multi-QR round-trip for script > single QR limit; no Redis write on LAN path; E2E pass

**Validation steps:** `pytest tests/pairing/` (LAN routes); `npm test`; Playwright LAN + multi-QR specs

**Rollback considerations:** Feature flag `HANDOFF_LAN_ENABLED` / hide multi-QR; relay remains fallback

**Testing requirements:** Unit + API integration + Playwright on LAN IP (`PUBLIC_HOST`)

**Complexity:** L

**Operational impact:** Document hotspot `PUBLIC_HOST` in deploy README; LAN store is process-local (single replica)

#### Tasks — M7: Serverless large handoff

| ID | Description | Files | FR/NFR | Complexity | Status |
|----|-------------|-------|--------|------------|--------|
| M7-T1 | ADR 006 serverless handoff modes (LAN + multi-QR) | `.work/decisions/20260521-006-serverless-handoff-modes.md` | FR-11, FR-12 | S | done 2026-05-21 |
| M7-T2 | LAN one-shot API: in-memory store, POST create + GET once, rate limit | `api/src/pairing/lan.py`, `api/src/pairing/routes.py`, tests | FR-11, NFR-11, R12 | L | pending |
| M7-T3 | LAN handoff UI: create URL for `PUBLIC_HOST`, mobile fetch route | `frontend/src/pairing/LanHandoff*.tsx`, `HandoffCreate.tsx` | FR-11 | M | pending |
| M7-T4 | Multi-QR: chunk encoder/decoder + “scan N of M” UX | `frontend/src/pairing/qrChunk*.ts`, `MultiQr*.tsx` | FR-12, R11 | L | pending |
| M7-T5 | Handoff mode router: single QR → multi-QR → LAN → relay fallback | `frontend/src/pairing/HandoffCreate.tsx`, `qrThreshold.ts` | FR-06, FR-11, FR-12 | M | pending |
| M7-T6 | Amend prompter-ui SPEC (R11 LAN, R12 multi-QR) | `.work/features/prompter-ui/20260520-SPEC.md` | FR-11, FR-12 | S | pending |
| M7-T7 | Playwright E2E: LAN + multi-QR on hotspot URL | `frontend/tests/e2e/handoff-lan.spec.ts`, `handoff-multi-qr.spec.ts` | FR-11, FR-12 | L | pending |
| M7-T8 | Document QR fragment vs QR encode limits (D14) | `qrConstants.ts`, deploy/README | U8, R9 | S | pending |
| M7-T9 | MOD-06 security review + `@code-verify milestone` | `.work/context/`, NEXT.md | NFR-11, W3 | S | pending |

---

### M8: Adaptive teleprompter (mic VAD + read zone)

**Objective:** Optional player sync: browser mic + voice-activity detection keeps the estimated read line in an upper-middle **read zone** while speaking; pauses on silence; skims meta lines at 2×. Defaults off. No cloud STT.

**Scope in:** `frontend/src/prompter/adaptive/`, Settings toggles, player mic button, script meta parser, markdown `tp-meta` blockquote, editor hints  
**Scope out:** Cloud STT, Web Speech API, word alignment, API/backend changes, npm speech ML libs

**Dependencies:** M4 complete (player scroll); approved `.work/features/adaptive-teleprompter/20260523-SPEC.md` + amendment 01; markdown-render amendment 01

**Deliverables:** Settings (`adaptiveEnabled`, `adaptiveAutoSync`); player mic toggle; VAD scroll controller; meta line parser; tests

**Acceptance criteria:** adaptive SPEC R1–R19 + amendment 01 (read zone 35–48%, baseline speed while speaking, pause on silence, 2× meta); FR-13; NFR-12

**Validation steps:** `npm test` (adaptive + markdown); Playwright mic mock + a11y touch target; `@code-verify milestone`

**Rollback considerations:** Settings default off; revert frontend only

**Testing requirements:** vitest (parser, VAD mock, read zone); Playwright (permission denied, mic button)

**Complexity:** L

**Operational impact:** None (client-only); document mic/Web Audio in `DOCS_TECH_STACK.md`

#### Tasks — M8: Adaptive teleprompter

| ID | Description | Files | FR/NFR | Complexity | Status |
|----|-------------|-------|--------|------------|--------|
| M8-T1 | prompter-ui SPEC amendment: adaptive settings + player mic | `.work/features/prompter-ui/20260523-SPEC-amendment-02-adaptive.md` | FR-13 | S | pending |
| M8-T2 | Script line parser + meta classification (`spoken` \| `meta`) | `frontend/src/prompter/adaptive/parseScriptLines.ts`, `types.ts`, tests | FR-13 | M | pending |
| M8-T3 | markdown-render amendment: `tp-meta` on blockquote + `isMetaSourceLine` | `frontend/src/markdown/render.ts`, `sanitize.ts`, tests | FR-13, NFR-03 | M | pending |
| M8-T4 | Web Audio VAD hook (`getUserMedia` + AnalyserNode) | `frontend/src/prompter/adaptive/useVoiceActivity.ts`, tests | FR-13, NFR-12, R13 | M | pending |
| M8-T5 | Adaptive scroll: read zone (42% center, 35–48% band), pause on silence, 2× meta | `frontend/src/prompter/adaptive/useAdaptiveScroll.ts`, integrate `Player.tsx` / `useScroll.ts` | FR-13, G7 | L | pending |
| M8-T6 | Settings UI: Adaptive teleprompter + Auto-sync toggles; persist settings | `frontend/src/prompter/Settings.tsx`, `storage.ts`, `en.ts` | FR-13 | M | pending |
| M8-T7 | Player mic button (primary toolbar); sync toggle; permission UX | `frontend/src/prompter/PlayerControls.tsx`, styles | FR-13, NFR-07, R13 | M | pending |
| M8-T8 | Editor hint copy for meta markup syntax | `frontend/src/prompter/Editor.tsx`, `en.ts` | FR-13 | S | pending |
| M8-T9 | vitest: parser fixtures, VAD mock, read zone band, meta 2×, I1–I2 | `frontend/tests/adaptive/*.test.ts` | FR-13 | M | pending |
| M8-T10 | Playwright: mic on toolbar; permission denied; auto-sync on play (mocked) | `frontend/tests/e2e/adaptive-player.spec.ts` | FR-13, NFR-07 | M | pending |
| M8-T11 | MOD-06 review + `@code-verify milestone` | `.work/context/`, NEXT.md | NFR-12, MOD-06 | S | pending |
...
| AC-10 | All FR-01–FR-13 traced to done tasks | traceability matrix |
| AC-11 | LAN handoff (J2c) completes; no Redis key; single GET | pytest + E2E |
| AC-12 | Multi-QR handoff (J2d) completes without API body | E2E + network audit |
| AC-13 | Adaptive sync (J1b): read zone + pause on silence; fixed speed when sync off; mic local-only | M8-T9, M8-T10, AC-6 style log audit |
...
| M7 | `pytest tests/pairing/ -q` (LAN routes) | exit 0 |
| M7 | `npm test`; Playwright handoff-lan + handoff-multi-qr | exit 0 |
| M7 | `@code-verify milestone` | pass |
| M8 | `npm test` (adaptive + markdown) | exit 0 |
| M8 | Playwright adaptive-player (mocked mic) | exit 0 |
| M8 | `@code-verify milestone` | pass |
...
| M7 | Disable LAN via env; hide multi-QR; relay unchanged |
| M8 | Settings default off; adaptive module tree revert |
...
| Mic / adaptive privacy | M8-T4, M8-T7 | standard tier + a11y audit |
...
| D11 | 2026-05-21 | v1 relay retained for large scripts; v2 additive | Replace relay retroactively |
| D12 | 2026-05-21 | M7: LAN one-shot then multi-QR (owner approved) | WebRTC in M7 |
| D13 | 2026-05-21 | WebRTC deferred to v2.1+ | Signaling without ADR |
| D14 | 2026-05-21 | Separate fragment threshold vs QR encode limit | Single combined threshold |
| D15 | 2026-05-23 | M8: adaptive teleprompter — Web Audio VAD only; read zone; no cloud STT | Full STT alignment; cloud speech APIs |
| D16 | 2026-05-23 | Meta lines 2× skim; read zone 35–48% (center 42%) | 3× meta; speech-rate matching |
...
| G7 | FR-13 | 004 | adaptive-teleprompter | M8-T4, M8-T5, M8-T7 | M8-T9, AC-13 |

**FR traceability:** FR-01–FR-13 each map to ≥1 task (see trace file). **Gap:** none at plan time.
...
| M8 vs adaptive SPEC | pass | FR-13 maps to M8-T1–T11; SPEC + amendments referenced |
...
**Integrity score:** pass (M8 revise 2026-05-23)

**Approval gate:** Plan **Approved** v1.2 — M8 additive milestone; M1–M7 shipped per HANDOFF.
...
| 1.1 | 2026-05-21 | M7 LAN + multi-QR (ADR 006) |
| 1.2 | 2026-05-23 | M8 adaptive teleprompter (FR-13); G7; NFR-12; R13 |
